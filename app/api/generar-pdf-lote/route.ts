...
import sql from '@/app/lib/db'; // <--- Cambiado a tu conexión local
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const chunk of chunks) {
      // Usamos Promise.all para procesar el chunk
      ...
      for (const [rRes, evRes] of chunkResults) {
        // CAMBIO CRÍTICO: rRes y evRes ya son arrays, no tienen .rows
        if (!rRes || rRes.length === 0) continue;
        const row = rRes[0];

        const evMap = new Map<number, any[]>();
        for (const ev of evRes) { // <--- evRes es el array directo
    ...
    const nombreArchivo = `LOTE-${new Date().toISOString().slice(0, 10)}-${Math.floor(Math.random() * 900 + 100)}`;
    ...
    console.error('[generar-pdf-lote] Error:', err);
