...
type EvidenceEntry = { id: number; observation: string; geoRef: GeoRef | null; photo: string | null };
type ChecklistItem = {
  id: number | string;
  pregunta: string;
  respuesta: string;
...
function getEvidences(item: ChecklistItem): Array<{ obs: string; lat: string; lon: string; foto: string | null }> {
  if (Array.isArray(item.evidence) && item.evidence.length > 0) {
    return item.evidence
      ...
      .map(e => ({ obs: e.observation || '', lat: e.geoRef?.lat ?? '', lon: e.geoRef?.lon ?? '', foto: e.photo ?? null }));
  }
  const tieneAlgo = !!(item.observacion?.trim() || item.geoRef || item.foto);
  if (!tieneAlgo) return [];
  return [{ obs: item.observacion || '', lat: item.geoRef?.lat ?? '', lon: item.geoRef?.lon ?? '', foto: item.foto ?? null }];
}
...
// ══════════════════════════════════════════════════════════════════════════════
//  ESTRATEGIA DE PAGINACIÓN DEL ÍNDICE (dos pasadas)
//
//  El índice necesita mostrar números de página del contenido, pero el
//  contenido aún no existe cuando se empieza a generar el PDF.
//
//  Solución:
//  1. Calcular cuántas páginas de índice se necesitan (MAX_ITEMS_POR_PAG = 35).
//  2. PRE-RESERVAR esas páginas en blanco al inicio.
//  3. Generar el contenido a partir de la página (numPagsIndice + 1).
//     → Los números de página del contenido ya son correctos porque las
//       páginas de índice están físicamente antes.
//  4. Volver a las páginas reservadas (doc.setPage) y dibujar el índice
//     con los números de página reales.
// ══════════════════════════════════════════════════════════════════════════════
export async function generarPDFCombinado(lista: QueuedForm[]): Promise<string> {
  ...
  const MAX_ITEMS_POR_PAG = 35;   // máx. entradas por página de índice
  const Y_INICIO_ITEMS    = 52;   // mm donde empieza la lista (tras encabezado)
  const LINE_H            = 6;    // mm por línea de ítem

  // ── Número de páginas de índice necesarias ────────────────────────────
  const numPagsIndice = Math.max(1, Math.ceil(lista.length / MAX_ITEMS_POR_PAG));
  ...
  // ── PRE-RESERVAR páginas de índice ────────────────────────────────────
  // La primera página ya existe; añadimos las adicionales si hace falta.
  for (let i = 1; i < numPagsIndice; i++) doc.addPage();

  // ── Map: categoria → número de página donde empieza ──────────────────
  const paginasPorCat = new Map<string, number>();

  // ══════════════════════════════════════════════════════════════════════
  //  PASADA 1 — CONTENIDO (comienza en la página numPagsIndice + 1)
  // ══════════════════════════════════════════════════════════════════════
  let esPrimerCat = true;

  for (const cat of ordenCats) {
    const formsGrupo = grupos.get(cat)!;

    // Siempre nueva página para cada categoría
    doc.addPage();
    if (esPrimerCat) esPrimerCat = false;

    // Registrar en qué página empieza esta categoría
    paginasPorCat.set(cat, doc.getNumberOfPages());

    // ── Encabezado de sección ─────────────────────────────────────────
    let y = 20;
    doc.setFont('helvetica', 'bold').setFontSize(12).setTextColor(...C.black);
    ...
    const fechaBase = formsGrupo[0].fechaCaptura instanceof Date
      ? formsGrupo[0].fechaCaptura : new Date(formsGrupo[0].fechaCaptura);
    ...
    formsGrupo.forEach(form => {
      ...
      if (mostrarBandaSub && subTipo?.toUpperCase().trim() !== lastSeccion) {
        bodyRows.push([{
          content: subTipo,
          colSpan: NCOLS,
          styles: { halign: 'center', fontStyle: 'bold', fontSize: 9.5, fillColor: C.darkGray, textColor: C.white, cellPadding: { top: 3.5, bottom: 3.5, left: 4, right: 4 } },
        }]);
      ...
      checklist.forEach(item => {
        ...
        if (seccion && seccionUp !== lastSeccion.toUpperCase().trim() && seccionUp !== subTipoUp && seccionUp !== catUp) {
          bodyRows.push([{
            content: seccion,
            colSpan: NCOLS,
            styles: { halign: 'left', fontStyle: 'bold', fontSize: 8.5, fillColor: C.lightGray, textColor: C.black, cellPadding: { top: 3, bottom: 3, left: 8, right: 4 } },
          }]);
          rowTypes.push({ type: 'sep' });
        ...
        evidences.forEach((ev, ei) => {
          const esExtra = ei > 0;
          bodyRows.push([
            { content: esExtra ? '' : String(item.id),                  styles: { halign: 'center', fontSize: 7.5, fontStyle: esExtra ? 'normal' : 'bold', valign: 'middle', fillColor: esExtra ? C.rowBg : C.white } },
            { content: esExtra ? `↳ Evidencia ${ei + 1}` : item.pregunta, styles: { fontSize: 7.5, fontStyle: esExtra ? 'italic' : 'normal', valign: 'middle', fillColor: esExtra ? C.rowBg : C.white, textColor: esExtra ? C.midGray : C.black } },
            { content: ev.lat, styles: { halign: 'center', fontSize: 7, textColor: ev.lat ? C.green : C.black, valign: 'middle', fillColor: esExtra ? C.rowBg : C.white } },
            { content: ev.lon, styles: { halign: 'center', fontSize: 7, textColor: ev.lon ? C.green : C.black, valign: 'middle', fillColor: esExtra ? C.rowBg : C.white } },
            { content: ev.obs, styles: { fontSize: 7.5, valign: 'top', fillColor: esExtra ? C.rowBg : C.white } },
            { content: '',     styles: { halign: 'center', valign: 'middle', fillColor: esExtra ? C.rowBg : C.white, minCellHeight: 24 } },
          ]);
    ...
    autoTable(doc, {
      ...
      head: [
        [{ content: cat, colSpan: NCOLS, styles: { halign: 'center', fontStyle: 'bold', fontSize: 11, fillColor: C.black, textColor: C.white, cellPadding: { top: 5, bottom: 5, left: 4, right: 4 } } }],
        [
      ...
      headStyles: { fillColor: C.midGray, textColor: C.white, fontStyle: 'bold', fontSize: 7.5, halign: 'center', cellPadding: 2.5 },
      body: bodyRows,
      theme: 'grid',
      styles: { lineWidth: 0.15, lineColor: [180, 180, 180] as any, fontSize: 7.5, cellPadding: 2.5, overflow: 'linebreak', minCellHeight: 9 },
      columnStyles: {
      ...
      didDrawCell: (data: any) => {
        ...
        try {
          ...
          doc.addImage(foto, fmt, data.cell.x + (CW.foto - dw) / 2, data.cell.y + (data.cell.height - dh) / 2, dw, dh, `img_${_imgAlias++}`, 'FAST');
        } catch {
          try {
            doc.setDrawColor(200, 200, 200).setFillColor(245, 245, 245);
            doc.rect(data.cell.x + 1, data.cell.y + 1, CW.foto - 2, data.cell.height - 2, 'FD');
    ...
    const conEv      = formsGrupo.reduce((acc, f) => acc + (f.checklist as ChecklistItem[]).filter(tieneEvidencia).length, 0);
  ...
  for (let i = 1; i <= totalPaginas; i++) {
    ...
    try { doc.addImage('/logo_fonatur.png', 'PNG', (pageW - 140) / 2, (pageH - 40) / 2, 140, 40); } catch { /* noop */ }
    doc.restoreGraphicsState();
    // Número de página en todas excepto las de índice
    if (i > numPagsIndice) {
      doc.setFont('helvetica', 'normal').setFontSize(7).setTextColor(150, 150, 150);
      doc.text(`${folio}`, mg, pageH - 6);
      doc.text(`Pág. ${i} / ${totalPaginas}`, pageW - mg, pageH - 6, { align: 'right' });
      doc.setTextColor(0, 0, 0);
    }
  }

  // ══════════════════════════════════════════════════════════════════════
  //  PASADA 2 — DIBUJAR ÍNDICE EN PÁGINAS PRE-RESERVADAS
  //
  //  Ahora que el contenido existe, sabemos exactamente en qué página
  //  empieza cada categoría. Volvemos a las páginas 1..numPagsIndice
  //  y dibujamos el mismo estilo de lista que ya tenían, pero con
  //  paginación automática al superar MAX_ITEMS_POR_PAG = 35 ítems.
  // ══════════════════════════════════════════════════════════════════════

  // ── Función auxiliar: dibuja el encabezado de una página de índice ────
  const dibujarEncabezadoIndice = (pagIndex: number) => {
    doc.setPage(pagIndex);

    // Título principal
    let y = 28;
    doc.setFont('helvetica', 'bold').setFontSize(15).setTextColor(...C.black);
    doc.text('REPORTE COMBINADO – CIP ACAPULCO-COYUCA', mg, y); y += 7;

    doc.setFont('helvetica', 'normal').setFontSize(10).setTextColor(...C.midGray);
    doc.text(`Folio: ${folio}`, mg, y); y += 4;

    doc.setDrawColor(...C.lightGray).setLineWidth(0.5).line(mg, y, pageW - mg, y); y += 7;

    doc.setFont('helvetica', 'bold').setFontSize(11).setTextColor(...C.black);
    doc.text('Contenido:', mg, y); y += 7;

    doc.setTextColor(0, 0, 0);
    return y; // Y donde empiezan los ítems
  };

  // ── Dibujar ítems del índice con salto de página automático ──────────
  let paginaActualIdx = 1;
  let yIdx = dibujarEncabezadoIndice(paginaActualIdx);
  let itemsEnPaginaActual = 0;

  lista.forEach((form, idx) => {
    // ¿Necesitamos saltar a la siguiente página de índice?
    if (itemsEnPaginaActual >= MAX_ITEMS_POR_PAG) {
      paginaActualIdx++;
      yIdx = dibujarEncabezadoIndice(paginaActualIdx);
      itemsEnPaginaActual = 0;
    }

    doc.setPage(paginaActualIdx);
    ...
    const sector = form.formData.sector || '—';
    const tramo  = form.formData.Tramo  || '—';

    // Texto exactamente igual al screenshot:
    // "N. CATEGORIA › SUB-TIPO  —  SECTOR / TRAMO  (FECHA)"
    const linea = `${idx + 1}. ${form.categoria}${sub}  —  ${sector} / ${tramo}  (${fecha})`;

    doc.setFont('helvetica', 'normal').setFontSize(10).setTextColor(...C.black);
    doc.text(linea, mg + 4, yIdx);
