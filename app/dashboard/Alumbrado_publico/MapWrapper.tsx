...
// Cargamos el mapa con SSR deshabilitado para evitar el error 'window is not defined'
...
export default function MapWrapper({ gps, reportes }: { gps: any, reportes: Reporte[] }) {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedSector, setSelectedSector] = useState("Todos");

  // Optimizamos la búsqueda para que no se trabe el teclado en móviles
  const deferredSearch = useDeferredValue(searchTerm);

  const reportesValidos = useMemo(() => {
    return reportes.filter(r => r.latitud !== null && r.longitud !== null);
  }, [reportes]);

  const sectores = useMemo(() => {
    return ["Todos", ...Array.from(new Set(reportesValidos.map(r => r.sector)))];
  }, [reportesValidos]);
  ...
  const reportesFiltrados = useMemo(() => {
    ...
    return reportesValidos.filter(r => {
      ...
      const matchSearch = r.folio?.toLowerCase().includes(term) || r.id?.toString().includes(term);
      return matchSector && matchSearch;
    });
  }, [deferredSearch, selectedSector, reportesValidos]);
    ...
    <div className="flex flex-col gap-4 h-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="relative">
          ...
          <select
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-sm appearance-none"
      ...
      <div className="flex-1 min-h-[500px] relative rounded-xl overflow-hidden border border-slate-200">
...
//   // 1. Pre-filtrar: Quedarnos SOLO con los reportes que tienen latitud y longitud válidas
//   const reportesValidos = useMemo(() => {
//     return reportes.filter(r => {
//       // Usamos los nombres correctos de tu BD y verificamos que no sean null
//       return r.latitud !== null && r.longitud !== null; 
//     });
//   }, [reportes]);

//   // 2. Extraer sectores únicos de los reportes VÁLIDOS
//   const sectores = useMemo(() => {
//     const listaSectores = reportesValidos.map(r => r.sector);
//     return ["Todos", ...Array.from(new Set(listaSectores))];
//   }, [reportesValidos]);

//   // 3. Lógica de filtrado combinada (Buscador + Sector)
//   const reportesFiltrados = useMemo(() => {
...
//     });
//   }, [searchTerm, selectedSector, reportesValidos]);
...
//           Mostrando <span className="font-bold text-blue-600">{reportesFiltrados.length}</span> puntos en el mapa de {reportes.length} reportes totales.
//         </p>
//       </div>
