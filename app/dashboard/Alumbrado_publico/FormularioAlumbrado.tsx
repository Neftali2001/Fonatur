...
const checklist_inicial = (tipo: string): ChecklistItem[] => {
  ...
  return preguntas.map((p, i) => ({ id: i + 1, pregunta: p, respuesta: "", observacion: "", geoRef: null }));
};
...
const FormularioAlumbrado: React.FC<FormularioProps> = ({ reporteParaEditar }) => {
  ...
  checklist_inicial(reporteParaEditar?.tipoAlumbrado ?? 'Alumbrado Público') 
);
  // const [checklist, setChecklist] = useState<ChecklistItem[]>(
  //   parsearChecklist(reporteParaEditar?.checklist) ?? checklist_inicial(formData.tipoAlumbrado)
  // );
  // const [checklist, setChecklist] = useState<ChecklistItem[]>(
...
const handleTipoAlumbradoChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    const nuevoTipo = e.target.value;
    
    // 1. Preguntamos al usuario si está seguro de cambiar, ya que se borrarán las respuestas actuales
    const preguntasRespondidas = checklist.filter(i => i.respuesta !== "").length;
    if (preguntasRespondidas > 0) {
      if (!window.confirm("Cambiar el tipo de alumbrado borrará las respuestas actuales. ¿Deseas continuar?")) {
        return; // Si cancela, no hacemos nada
      }
    }

    // 2. Actualizamos el formulario y reseteamos el checklist
    setFormData(prev => ({ ...prev, tipoAlumbrado: nuevoTipo }));
    setPreguntaActual(0);
    setChecklist(checklist_inicial(nuevoTipo));
  }, [checklist]);
...
const limpiarFormulario = useCallback(() => {
  setPreguntaActual(0);
  // Le pasamos el tipo que tiene el formulario en ese momento
  setChecklist(checklist_inicial(formData.tipoAlumbrado)); 
  setFotos(FOTOS_INICIAL);
  setGps({ lat: null, lon: null, precision: '--' });
}, [formData.tipoAlumbrado]); // Agrega formData.tipoAlumbrado a las dependencias
  // const limpiarFormulario = useCallback(() => {
  ...
  const guardarCuestionario = useCallback(async (): Promise<string> => {
    ...
    if (reporteParaEditar?.id) {
      const id = await actualizarReporte(reporteParaEditar.id.toString(), fd, checklist, gps, fotos);
      alert("¡Reporte actualizado!");
      return id;
    } else {
      const id = await crearReporte(fd, checklist, gps, fotos);
  ...
  try {
    // Guarda en BD y recibe el id real del registro
    ...
    // Agregar a la cola global con el id real devuelto por la BD
    addToQueue({
      id:           reporteId,           // ← id real, no formData.id
  ...
  const generarPDFMultiples = async (lista: any[]) => {
    ...
    for (let index = 0; index < lista.length; index++) {
      ...
      doc.setFont("helvetica", "bold").setTextColor(211, 84, 0).text(`TIPO LUMINARIA: ${form.formData.tipoAlumbrado.toUpperCase()}`, margin, y);
    ...
    <div className="relative">
      <div className="max-w-5xl mx-auto font-sans bg-[#eef2f6] p-4 sm:p-8 space-y-6 text-gray-700">
        ...
        <div className="p-2 space-y-6">
          ...
          {/* TIPO MANTENIMIENTO */}
          {/* TIPO MANTENIMIENTO Y TIPO ALUMBRADO */}
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            {/* NUEVO MENÚ DESPLEGABLE: TIPO DE ALUMBRADO */}
            <div className="flex flex-col gap-2 w-full sm:w-1/2">
              <label className="text-xs tracking-wider uppercase text-[#d35400] font-bold">Tipo de Alumbrado / Luminaria</label>
              <select 
                value={formData.tipoAlumbrado} 
                onChange={handleTipoAlumbradoChange}
                className="px-3 py-2 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold focus:outline-none focus:ring-2 focus:ring-[#e67e22]/30"
              >
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6">
                {(["SI", "NO"] as const).map(val => (
                  <label key={val} className={`cursor-pointer flex-1 py-4 px-6 rounded-xl border-2 text-center font-bold transition-all ${
                    ...
                    {val === "SI"
                      ? (itemActual.id >= 9 ? "BUENO" : "CUMPLE (SÍ)")
                      : (itemActual.id >= 9 ? "MALO"  : "NO CUMPLE (NO)")}
                  </label>
                ))}
              </div>
...
// }
// interface GpsCoords { lat: string | null; lon: string | null; precision: string; }
// interface FormData { id: string; sector: string; Tramo: string; accesoPublico: string; tipoMantenimiento: string; categoria: string; }
// interface Fotos { [key: string]: string | null; }
// interface GeoRef { lat: string; lon: string; precision: string; timestamp: string; }
// interface ChecklistItem { id: number; pregunta: string; respuesta: string; observacion: string; geoRef?: GeoRef | null; }
