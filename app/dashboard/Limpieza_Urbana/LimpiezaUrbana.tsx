...
// ================= COMPONENTE =================
const LimpiezaUrbana: React.FC<FormularioProps> = ({ reporteParaEditar }) => {
    ...
    geoRefWatchId.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        setChecklist(prev => prev.map(item =>
          item.id === itemId ? { ...item, geoRef: { lat: latitude.toFixed(6), lon: longitude.toFixed(6), precision: `${accuracy.toFixed(1)}m`, timestamp } } : item
        ));
  ...
  const generarPDFMultiples = async (lista: any[]) => {
    ...
    for (let index = 0; index < lista.length; index++) {
      ...
      y += 4;
      doc.setLineWidth(0.6).line(margin, y, pageWidth - margin, y);
      y += 8;
      ...
      doc.text("1. LISTA DE VERIFICACIÓN – LIMPIEZA URBANA", margin, y); y += 5;
