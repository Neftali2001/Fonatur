...
const INSTA360_IMAGE_EXTS = ['.insp'];
const INSTA360_VIDEO_EXTS = ['.insv', '.lrv'];
...
// Patrón diagonal en SVG repetido en toda la superficie del canvas.
// pointer-events:none → no interfiere con el arrastre ni el scroll.
// select-none → el texto no se puede seleccionar ni copiar.
// Al tomar una captura de pantalla el patrón queda grabado en la imagen.
const WatermarkOverlay: React.FC<{ projectName: string }> = ({ projectName }) => {
  const wm = `360° ${projectName.toUpperCase()}`;
  return (
    <div
      aria-hidden
      className="absolute inset-0 z-20 pointer-events-none select-none overflow-hidden"
    >
      {/* ── Patrón diagonal tiled ─────────────────────────────────────────── */}
      <svg
      ...
      {/* ── Badge esquina inferior izquierda (encima de los botones) ─────── */}
      <div className="absolute bottom-12 left-3 flex items-center gap-2 rounded-xl px-3 py-2"
        ...
        <svg className="w-6 h-6 flex-shrink-0" viewBox="0 0 24 24" fill="none">
          ...
          <ellipse cx="12" cy="12" rx="5" ry="10" stroke="rgba(255,255,255,0.35)" strokeWidth="1" fill="none"/>
          <line x1="2" y1="12" x2="22" y2="12" stroke="rgba(255,255,255,0.35)" strokeWidth="1"/>
          <text x="12" y="16" textAnchor="middle" fontFamily="monospace" fontWeight="900"
            fontSize="7" fill="rgba(255,255,255,0.80)" letterSpacing="0.5">360°</text>
        </svg>
        <div className="flex flex-col leading-none gap-0.5">
          <span style={{ fontSize: 9, fontWeight: 900, color: 'rgba(255,255,255,0.85)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
            CIP ACAPULCO-COYUCA
          </span>
          <span style={{ fontSize: 7.5, fontWeight: 700, color: 'rgba(255,255,255,0.45)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
            Registro Visual Inmersivo
          </span>
        </div>
      </div>

      {/* ── Texto esquina superior derecha (junto a la brújula) ──────────── */}
      {/* Se posiciona debajo de la brújula (top-14) para no solaparse */}
      <div className="absolute top-14 right-2 flex flex-col items-end gap-0.5">
        <span style={{ fontSize: 8, fontWeight: 900, color: 'rgba(255,255,255,0.40)', letterSpacing: '0.18em', textTransform: 'uppercase' }}>
...
const Viewer360: React.FC<Viewer360Props> = ({
  ...
  const threeRef   = useRef<{ scene: any; camera: any; renderer: any; sphere: any; animId: number } | null>(null);
  ...
  const initViewer = useCallback(async (entry: PanoramaEntry) => {
    ...
    // ── Shader dual-fisheye para .insv / .lrv ─────────────────────────────
    const vertexShader = `
      varying vec2 vUv;
      void main() {
        vUv = uv;
  ...
  function buildFallbackTexture(THREE: any) {
    ...
    grd.addColorStop(0, '#0f172a'); grd.addColorStop(0.4, '#065f46'); grd.addColorStop(1, '#1e293b');
  ...
  useEffect(() => {
    stateRef.current.lon = 0; stateRef.current.lat = 0;
  ...
  const onMouseMove = useCallback((e: MouseEvent) => {
    ...
    stateRef.current.lon -= (e.clientX - prevMouseRef.current.x) * 0.15;
    stateRef.current.lat  = Math.max(-85, Math.min(85, stateRef.current.lat + (e.clientY - prevMouseRef.current.y) * 0.15));
    prevMouseRef.current = { x: e.clientX, y: e.clientY };
  }, []);
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1 && prevTouchRef.current) {
      stateRef.current.lon -= (e.touches[0].clientX - prevTouchRef.current.x) * 0.18;
      stateRef.current.lat  = Math.max(-85, Math.min(85, stateRef.current.lat + (e.touches[0].clientY - prevTouchRef.current.y) * 0.18));
      prevTouchRef.current = { x: e.touches[0].clientX, y: e.touches[0].clientY };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImages(prev => { const next = [...prev, ...entries]; setActiveIdx(next.length - 1); return next; });
  };
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-md overflow-hidden">
            ...
            >
              ...
              {showHotspots && (
                <div className="absolute w-6 h-6 border-2 border-orange-400 rounded-full bg-orange-400/30 cursor-pointer z-10 animate-ping" style={{ top: '45%', left: '38%' }} />
              )}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-md overflow-hidden">
            ...
            <div className="px-4 py-3 flex flex-col gap-2">
              {[
                ...
                { k: 'Archivo activo',v: active?.label  ?? '—',                c: 'text-orange-500 font-bold' },
              ].map((r, i) => (
        ...
        <aside className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-md overflow-hidden">
            ...
            <div className="relative m-3 border-2 border-dashed border-slate-300 rounded-xl p-5 flex flex-col items-center gap-2 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-400 transition cursor-pointer"
              ...
              <input type="file" accept="image/*,video/*,.insp,.insv,.lrv" multiple
                onChange={handleFileUpload} onClick={e => { (e.target as HTMLInputElement).value = ''; }}
