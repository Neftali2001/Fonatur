...
import MapWrapper from '@/app/dashboard/Alumbrado_publico/MapWrapper'; 
...
async function getReportes(): Promise<Reporte[]> {
  try {
    // Eliminamos el { rows } porque la librería 'postgres' devuelve el array directamente
    const reportes = await sql`
      SELECT id, folio, sector, latitud::float, longitud::float 
      FROM reportes_alumbrado
    `;
    
    return reportes as unknown as Reporte[];
  } catch (error) {
