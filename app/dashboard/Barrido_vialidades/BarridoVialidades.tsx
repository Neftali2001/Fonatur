...
// ================= CONSTANTES (fuera del componente = sin re-creación) =================
...
const BarridoVialidades: React.FC<FormularioProps> = ({ reporteParaEditar }) => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    categoria:         reporteParaEditar?.categoria         ?? 'BARRIDO VIALIDADES',
  });
  ...
  const generarPDFMultiples = async (lista: any[]) => {
    ...
    for (let index = 0; index < lista.length; index++) {
      ...
      doc.text(`Hora: ${horaStr}`, pageWidth - 50, y);
      y += 6;
      ...
      doc.text("1. LISTA DE VERIFICACIÓN – BARRIDO DE VIALIDADES", margin, y); y += 5;
      ...
      if (form.mapImage) {
        ...
        doc.text(`Ubicación: ${ubStr}`, margin, 27);
      ...
      if (imagenes.length > 0) {
        ...
        for (let i = 0; i < imagenes.length; i++) {
          ...
          doc.roundedRect(xPos + 1.5, yImg + 1.5, imgW, imgH, 2, 2, "F");
          doc.setFillColor(255, 255, 255);
          doc.roundedRect(xPos, yImg, imgW, imgH, 2, 2, "F");
          ...
          if (xPos + imgW + gapX + imgW <= pageWidth - margin) {
            xPos += imgW + gapX;
          } else {
            xPos = margin;
            yImg += imgH + captH + gapY;
          }
        }
      }
    }
    ...
    <div className="relative">
      <div className="max-w-5xl mx-auto font-sans bg-[#eef2f6] p-4 sm:p-8 space-y-6 text-gray-700">
        ...
        <div className="flex justify-center">
          <img src="/logo_fonatur.png" alt="Logo FONATUR" className="w-1/2" />
        </div>
        ...
        <div className="p-2 space-y-6">
          ...
          <div className="w-full max-w-4xl mx-auto">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 px-4 sm:px-6 py-4
                            bg-gradient-to-r from-white to-gray-50 rounded-2xl shadow-md border border-gray-100">
              {/* Sector */}
              <div className="flex flex-col gap-2 w-full sm:w-1/3">
                ...
                {formData.sector === "Otro" && (
                  <input type="text" placeholder="Sector personalizado..." value={sectorPersonalizado}
                    ...
                    className="mt-1 px-3 py-2 rounded-xl border border-gray-200 w-full" />
                )}
              </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <select value={formData.tipoMantenimiento}
              ...
              className={`px-3 py-2 rounded-xl border font-bold transition-colors ${
                formData.tipoMantenimiento === 'Urgente' ? 'bg-red-50 border-red-200 text-red-600' : 'bg-white border-gray-200 text-gray-700'
              }`}>
              <option value="">Seleccionar</option>
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-2xl shadow-md border border-emerald-200 p-6">
              ...
              <button onClick={obtenerUbicacion} disabled={cargandoGps}
                className={`relative w-full py-3 rounded-xl text-white font-bold flex items-center justify-center overflow-hidden transition-all shadow-lg ${
                  cargandoGps ? "bg-slate-800 cursor-not-allowed border border-emerald-900/50" : "bg-emerald-600 hover:bg-emerald-700 hover:-translate-y-0.5"
                }`}>
                {cargandoGps ? (
                  <>
                    <style>{`
                    ...
                    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-xl">
                      <div className="w-full h-12 bg-gradient-to-b from-transparent via-emerald-400/20 to-emerald-400/50 border-b-2 border-emerald-400"
                    ...
                    <div className="relative z-10 flex items-center gap-3">
                      <div className="relative flex items-center justify-center w-6 h-6">
                        <div className="absolute inset-0 border-2 border-transparent border-t-emerald-400 rounded-full"
          ...
          <section className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-200 max-w-2xl mx-auto w-full">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 sm:p-8 min-h-[300px] flex flex-col justify-center">
              ...
              <div className="mt-auto space-y-3">
                ...
                <input type="text" placeholder="Escribe aquí si hay algún detalle..."
                  value={itemActual.observacion}
                ...
                <div className="flex flex-col gap-2">
                  {itemActual.geoRef ? (
                    <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2.5">
                      ...
                      <button type="button" onClick={() => limpiarGeoRefItem(itemActual.id)}
                    ...
                    <div className="relative w-full overflow-hidden rounded-xl bg-slate-900 border border-emerald-800 py-3 px-4 flex items-center gap-3">
                      <style>{`
                        @keyframes georef-scan  { 0%{transform:translateY(-150%)} 100%{transform:translateY(350%)} }
                        @keyframes georef-spin  { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
                        @keyframes georef-pulse { 0%,100%{opacity:.6;transform:scale(1)} 50%{opacity:0;transform:scale(1.8)} }
                      `}</style>
                      <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-xl">
                        <div className="w-full h-8 bg-gradient-to-b from-transparent via-emerald-400/20 to-emerald-500/40 border-b border-emerald-400/60"
                      ...
                      <div className="ml-auto flex-shrink-0 flex items-end gap-[3px] h-5 relative z-10">
                        {[0.4, 0.65, 1].map((delay, i) => (
                          <div key={i} className="w-1 bg-emerald-400 rounded-sm"
                            style={{ height: `${40 + i * 25}%`, animation: `georef-pulse ${0.8 + i * 0.15}s ease-in-out infinite`, animationDelay: `${delay * 0.3}s` }} />
                        ))}
                      </div>
                    </div>
                ...
                {itemActual.respuesta === "NO" && !itemActual.geoRef && (
                  <p className="text-[11px] text-amber-600 font-medium">
                    ⚠️ Incidencia detectada. Considera geo-referenciar la ubicación exacta.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between mt-4">
              <button type="button" onClick={() => setPreguntaActual(p => Math.max(0, p - 1))}
                disabled={preguntaActual === 0}
                className="px-5 py-2.5 rounded-lg font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 disabled:opacity-40 disabled:cursor-not-allowed transition-all text-sm">
                ← Anterior
              </button>
              <button type="button" onClick={() => setPreguntaActual(p => Math.min(checklist.length - 1, p + 1))}
                disabled={preguntaActual === checklist.length - 1}
                className="px-5 py-2.5 rounded-lg font-bold text-white bg-slate-800 hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all text-sm">
                Siguiente →
              </button>
            </div>
          </section>
                  ...
                  <div className="relative border-2 border-dashed border-gray-300 rounded-xl p-4 flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 transition h-48 overflow-hidden">
                    {fotos[tipo] ? (
                      <>
                        ...
                        <button onClick={() => setFotos(prev => ({ ...prev, [tipo]: null }))}
...
export default BarridoVialidades;
...
// const BarridoVialidades: React.FC<FormularioProps> = ({ reportesIniciales, reporteParaEditar }) => {
...
//       doc.text("1. LISTA DE VERIFICACIÓN – BARRIDO DE VIALIDADES", margin, y);
//       y += 5;
...
//         // ✅ Observaciones + geoRef en una sola celda (sin fila separadora — no aplica aquí)
...
//   accesoPublico: string;
//   categoria: string; 
//   tipoMantenimiento: string; // ← AGREGAR
// }
...
//     "ESTADO DEL BOLARDO (Bueno/Regular/Malo)",
//     "FIGURAS LÚDICAS  (Bueno/Regular/Malo)",
//     "SUTITUCIÓN DE TAPAS DE CONCRETO DIFERENTES EMPRESAS (TELECOMUNICACIÓNES, CFE Y ALUMBRADO)"
//   ];
