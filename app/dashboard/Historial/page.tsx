...
import sql from '@/app/lib/db'; // <--- Cambiado para usar tu nueva conexión
import {
...
export default async function HistorialPage({ searchParams }: HistorialPageProps) {
  ...
  const categoriaFiltro = p?.categoria || '';
  ...
  const categoriaPattern = categoriaFiltro ? `%${categoriaFiltro}%` : '%';

  // ── Queries en paralelo ────────────────────────────────────────────────
  const [conteoResult, result, sectoresResult, categoriasResult, fechasConConteo, todosIdsResult] =
    await Promise.all([
      ...
      WHERE (folio ILIKE ${searchPattern}
        ...
        AND categoria ILIKE ${categoriaPattern}
    `,
    ...
    `,
      ...
      // Todos los IDs filtrados
      (query || mantenimiento || sectorFiltro || categoriaFiltro)
        ? sql`
          ...
          ORDER BY
            ...
            CASE WHEN ${sort} = 'desc' THEN fecha END DESC
        `
        : Promise.resolve([]) // <-- Cambiado de { rows: [] } a []
    ]);

  // EXTRACCIÓN DE DATOS: Quitamos el .rows ya que la librería postgres devuelve el array directamente
  const totalRegistros = Number(conteoResult[0].count);
  const totalPaginas = Math.ceil(totalRegistros / ITEMS_POR_PAGINA);
  const reportes = result as unknown as ReporteAlumbrado[];
  const sectores = sectoresResult.map((r: any) => r.sector as string);
  const categorias = categoriasResult.map((r: any) => r.categoria as string);
  const todosFiltradosIds = (todosIdsResult as any[]).map((r: any) => r.id.toString());
  ...
  const hayFiltros = !!(query || mantenimiento || sectorFiltro || categoriaFiltro);

  return (
    <main className="p-4 md:p-6 max-w-7xl mx-auto space-y-5">

      {/* FORMULARIO DE FILTROS */}
      <form method="GET" className="bg-white p-4 md:p-5 rounded-xl shadow-md border border-gray-100">
        <div className="flex flex-col lg:flex-row gap-3 items-end flex-wrap">
          {/* Búsqueda */}
          <div className="w-full flex-1 min-w-[180px]">
            <label className="text-[10px] uppercase font-bold text-slate-500 mb-1.5 block ml-1">Búsqueda</label>
            <div className="relative">
              <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={12} />
              <input type="text" name="query" defaultValue={query} placeholder="Folio, sector o tramo…"
                className="w-full pl-9 pr-4 py-2.5 md:py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-sm" />
            </div>
          </div>

          {/* Selector Sector */}
          <div className="w-full lg:w-52">
            <label className="text-[10px] uppercase font-bold text-slate-500 mb-1.5 block ml-1">Ubicación / Sector</label>
            <select name="sector" defaultValue={sectorFiltro} className="w-full px-3 py-2.5 md:py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none bg-white text-sm">
          ...
          <div className="w-full lg:w-48">
            <label className="text-[10px] uppercase font-bold text-slate-500 mb-1.5 block ml-1">Categoría</label>
            <select name="categoria" defaultValue={categoriaFiltro} className="w-full px-3 py-2.5 md:py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none bg-white text-sm">
          ...
          <div className="w-full lg:w-44">
            <label className="text-[10px] uppercase font-bold text-slate-500 mb-1.5 block ml-1">Mantenimiento</label>
            <select name="mantenimiento" defaultValue={mantenimiento} className="w-full px-3 py-2.5 md:py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none bg-white text-sm">
          ...
          <div className="flex gap-2 w-full lg:w-auto">
            <button type="submit" className="flex-1 lg:flex-none px-6 py-2.5 md:py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg shadow-sm text-sm transition-all">
              Aplicar
            </button>
            {hayFiltros && (
              <a href="/dashboard/Historial" className="flex-1 lg:flex-none text-center px-4 py-2.5 md:py-2 border border-gray-200 text-gray-600 hover:bg-gray-50 rounded-lg text-sm font-semibold transition-colors">
                Limpiar
              </a>
            )}
            <BotonActualizar />
      ...
      {hayFiltros && todosFiltradosIds.length > 0 && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p className="text-sm font-bold text-emerald-800">Generar PDF combinado con los reportes filtrados</p>
            <p className="text-[11px] text-emerald-600 mt-0.5">{todosFiltradosIds.length} reportes incluidos</p>
          </div>
          <GenerarPDFLote
            ids={todosFiltradosIds}
            totalCount={todosFiltradosIds.length}
            titulo={['REPORTE CIP', sectorFiltro, categoriaFiltro, mantenimiento, query].filter(Boolean).join(' · ')}
          />
        </div>
      )}
      ...
      <div className="bg-transparent md:bg-white rounded-none md:rounded-xl md:shadow-lg md:border md:border-gray-200 flex flex-col gap-4 md:gap-0">
        ...
        <div className="flex flex-col gap-4 md:gap-0">
          {reportes.map(reporte => {
            ...
            return (
              <div key={reporte.id} className="bg-white rounded-xl shadow-md border border-gray-200 p-4 md:p-0 md:rounded-none md:shadow-none md:border-b md:border-gray-100 md:grid md:grid-cols-[1.5fr_1fr_2fr_1fr_0.5fr_0.5fr_1fr] md:gap-4 md:items-center hover:bg-emerald-50/30 transition-colors group">
                {/* Folio + Fecha */}
                ...
                <div className="md:px-6 md:py-4 md:text-center flex items-center gap-2">
                  {reporte.latitud ? (
                    <a href={`https://www.google.com/maps?q=${reporte.latitud},${reporte.longitud}`}
                      target="_blank" rel="noreferrer" className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
                      <FaMapMarkerAlt size={14} />
                    </a>
                  ) : <span className="text-gray-400 italic text-[10px]">Sin GPS</span>}
                </div>
                ...
                <div className="md:px-3 md:py-4 md:text-center flex items-center justify-center">
                  ...
                  ) : <FaFilePdf size={14} className="text-gray-300" />}
                </div>

                {/* Acciones */}
                <div className="flex gap-2 justify-end md:px-6 md:py-4">
                  <Link href={`/dashboard/PineoA?editId=${reporte.id}`} className="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
                    <FaEdit size={14} />
                  </Link>
                  <form action={eliminarBind}>
                    <button type="submit" className="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors">
                      <FaTrash size={14} />
                    </button>
                  </form>
                </div>
        ...
        {reportes.length === 0 && (
          <div className="py-20 text-center flex flex-col items-center">
            ...
            <p className="text-slate-500 font-medium">No se encontraron reportes.</p>
          </div>
        )}

        {/* Paginación */}
        {totalPaginas > 1 && (
          <div className="px-4 md:px-6 py-4 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 md:mt-0 rounded-b-xl border-t border-gray-200">
            <p className="text-sm text-slate-500">Página {paginaActual} de {totalPaginas}</p>
            <div className="flex gap-2 w-full sm:w-auto">
              <Link href={`?${qs}&page=${paginaActual - 1}`} className={`flex-1 text-center px-4 py-2 rounded-lg text-sm font-semibold transition ${paginaActual <= 1 ? 'bg-gray-200 text-gray-400 pointer-events-none' : 'bg-white border hover:bg-gray-100'}`}>Anterior</Link>
              <Link href={`?${qs}&page=${paginaActual + 1}`} className={`flex-1 text-center px-4 py-2 rounded-lg text-sm font-semibold transition ${paginaActual >= totalPaginas ? 'bg-gray-200 text-gray-400 pointer-events-none' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}>Siguiente</Link>
            </div>
          </div>
        )}
      </div>
...
//   const mantenimiento    = p?.mantenimiento || '';
//   const sectorFiltro     = p?.sector        || '';
//   const categoriaFiltro  = p?.categoria     || '';   // ← NUEVO
//   const paginaActual     = Number(p?.page)  || 1;
//   const offset           = (paginaActual - 1) * ITEMS_POR_PAGINA;

//   const searchPattern   = `%${query}%`;
//   const sectorPattern   = sectorFiltro    ? `%${sectorFiltro}%`   : '%';
//   const categoriaPattern = categoriaFiltro ? `%${categoriaFiltro}%` : '%';  // ← NUEVO
...
//     // Categorías únicas → dropdown de Categoría  ← NUEVO
//     sql`
//       SELECT DISTINCT categoria
//       FROM reportes_alumbrado
//       WHERE categoria IS NOT NULL AND categoria <> ''
...
//     // Todos los IDs filtrados (sin LIMIT) → PDF combinado completo
//     (query || mantenimiento || sectorFiltro || categoriaFiltro)
//       ? sql`
//           SELECT id FROM reportes_alumbrado
//           WHERE (folio  ILIKE ${searchPattern}
...
//   const categorias         = categoriasResult.rows.map(r => r.categoria as string);  // ← NUEVO
...
//     ...(mantenimiento      ? { mantenimiento }              : {}),
//     ...(sectorFiltro       ? { sector: sectorFiltro }       : {}),
//     ...(categoriaFiltro    ? { categoria: categoriaFiltro } : {}),  // ← NUEVO
//   }).toString();

//   const hayFiltros = !!(query || mantenimiento || sectorFiltro || categoriaFiltro);  // ← NUEVO
...
//             {categoriaFiltro && <span className="px-2 py-0.5 bg-violet-50 text-violet-700 border border-violet-200 rounded-full text-[11px] font-semibold">🏷️ {categoriaFiltro}</span>}
...
//               sectorFiltro     ? `SECTOR: ${sectorFiltro}`       : '',
//               categoriaFiltro  ? `CATEGORÍA: ${categoriaFiltro}` : '',  // ← NUEVO
...
//       {/* ── LISTA ── */}
//       {/* Añadida columna PDF entre GPS y Acciones: grid-cols-[1.5fr_1fr_2fr_1fr_0.5fr_0.5fr_1fr] */}
//       <div className="bg-transparent md:bg-white rounded-none md:rounded-xl md:shadow-lg md:border md:border-gray-200 flex flex-col gap-4 md:gap-0">
//         <div className="hidden md:grid md:grid-cols-[1.5fr_1fr_2fr_1fr_0.5fr_0.5fr_1fr] gap-4 px-6 py-4 bg-slate-800 text-white uppercase text-[11px] tracking-wider rounded-t-xl">
//           <div>Folio / Fecha</div>
//           <div className="text-center">Tipo</div>
...
//                 {/* GPS + PDF + Acciones (mobile: todo junto) */}
//                 <div className="flex justify-between items-center bg-gray-50 md:bg-transparent -mx-4 -mb-4 p-4 md:m-0 md:p-0 md:contents rounded-b-xl md:rounded-none border-t border-gray-100 md:border-none">
...
//             <p className="text-sm text-slate-500">Página {paginaActual} de {totalPaginas}</p>
//             <div className="flex gap-2 w-full sm:w-auto">
//               <Link href={`?query=${query}&sort=${sort}&mantenimiento=${mantenimiento}&page=${paginaActual - 1}`}
//                 className={`flex-1 sm:flex-none text-center px-4 py-2.5 rounded-lg text-sm font-semibold transition ${
