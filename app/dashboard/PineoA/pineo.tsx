...
const CATALOGO: Record<string, Record<string, Seccion[]>> = {
  ...
  'MOBILIARIO URBANO': {
    ...
    '14. Escalera':       [{ titulo: '11. ESCALERA',     items: ['Escalera dañada'] }],
  },
};
...
const FormularioUnificado: React.FC<FormularioProps> = ({ reporteParaEditar }) => {
  ...
  const procesarFormularioActual = useCallback(async () => {
    try {
      ...
      // ── FIX: resolver sector/tramo "Otro" antes de añadir a la cola ──
      // Sin esto, la cola guarda sector='Otro' en lugar del texto escrito,
      // y el PDF muestra literalmente la palabra "Otro".
      const sectorFinal = formData.sector === 'Otro' ? sectorPersonalizado : formData.sector;
      const tramoFinal  = formData.sector === 'Otro' ? tramoPersonalizado  : formData.Tramo;
      const formDataResuelto = { ...formData, sector: sectorFinal, Tramo: tramoFinal };

      addToQueue({
        categoria:    formData.categoria,
        formData:     formDataResuelto,
  ...
  }, [addToQueue, checklist, formData, sectorPersonalizado, tramoPersonalizado, gpsVista, guardarCuestionario, limpiarFormulario]);
